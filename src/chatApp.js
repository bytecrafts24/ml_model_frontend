...
const ChatApp = () => {
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            socket.off('disconnect');
            socket.off('user_joined');
            socket.off('receive_message', handleMessage); // Ensure to remove the specific listener
        };
    }, [location]);
    const createRoom = () => {
        const newRoomId = Math.random().toString(36).substring(7);
                ...
                <div>
                    ...
                    <div className="border h-64 overflow-y-auto p-2 mb-4" style={{ borderColor: '#ccc' }}>
                        {messages.map((msg, index) => {
                            ...
                            return (
                                <div key={index} className={`mb-2 ${isCurrentUser ? 'text-right' : 'text-left'}`}>
                                    <span className={`inline-block p-2 rounded ${isCurrentUser ? 'bg-blue-500 text-white' : 'bg-gray-300'}`}>
                                        {msg}
                                    </span>
                                </div>
                            );
